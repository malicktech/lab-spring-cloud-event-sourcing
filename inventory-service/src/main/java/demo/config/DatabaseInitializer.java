...
public class DatabaseInitializer implements CommandLineRunner {
	...
	public void run(String... strings) throws Exception {
		...
		List<Product> products = Arrays.asList(
				...
				new Product("Best. Cloud. Ever. (T-Shirt, Men's Large)", "SKU-24642",
						"<p>Do you love your cloud platform? "
								...
								+ "Four nines of <em>stylability</em></p>",
						21.99),
				...
				new Product("Like a BOSH (T-Shirt, Women's Medium)", "SKU-34563",
						"<p>The BOSH Outer Shell (<strong>BOSH</strong>) "
								...
								+ "4 nines of <em>re-washability</em></p>",
						14.99),
				...
				new Product("We're gonna need a bigger VM (T-Shirt, Women's Small)", "SKU-12464",
						"<i>\"Mr. Vaughn, what we are dealing with here is "
								...
								+ "little containers, and that's all.\"</i>",
						13.99),
				...
				new Product("cf push awesome (Hoodie, Men's Medium)", "SKU-64233",
						"<p>One of the great natives of the cloud once said \"<em>"
								...
								+ "&nbsp; Five nines of <em>comfortability</em></p>",
						21.99))
		...
		Address warehouseAddress = new Address("875 Howard St", null, "CA", "San Francisco", "United States", 94103);

		Address shipToAddress = new Address("1600 Amphitheatre Parkway", null, "CA", "Mountain View", "United States",
				94043);
		...
		Set<Inventory> inventories = products.stream()
				.map(a -> new Inventory(IntStream.range(0, 9).mapToObj(x -> Integer.toString(new Random().nextInt(9)))
		...
		for (int i = 0; i < 10; i++) {
			inventoryRepository.save(products.stream()
					.map(a -> new Inventory(IntStream.range(0, 9)
							.mapToObj(x -> Integer.toString(new Random().nextInt(9))).collect(Collectors.joining("")),
							a, finalWarehouse, InventoryStatus.IN_STOCK))
					.collect(Collectors.toSet()));
		}
